...
import utils
from model import CRW, ClassificationModel
...
def train_one_epoch(model, optimizer, lr_scheduler, data_loader, device, epoch, print_freq,
    ...
    for step, ((video, orig), label) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        start_time = time.time()

        video = video.to(device)
        label = label.to(device)
        output, loss = model(video, label)
        loss = loss.mean()
        #
...
def main(args):
    ...
    traindir = os.path.join(args.data_path, 'train' if not args.fast_test else 'valid')
    ...
    model = ClassificationModel(args, vis=vis).to(device)
    # Load checkpoint.
        ...
        if args.model_type == 'scratch':
            ...
            for k, v in checkpoint['model'].items():
                if 'conv1.1.weight' in k or 'conv2.1.weight' in k:
                    ...
                    state[k] = v
            utils.partial_load(state, model.crw_model, skip_keys=['head'])
        else:
            utils.partial_load(checkpoint['model'], model.crw_model, skip_keys=['head'])

    for child in model.crw_model.children():
